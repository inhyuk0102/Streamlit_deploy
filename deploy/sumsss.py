...
def login_page():
    ...
    with col1:
        ...
        if (st.button("ㅁㄴㅇㄹ", use_container_width=True)):
            navigate_to('info')
...
def getUserInfo():
    ...
    with input:
        ...
        return grade, hackgi, department
#######################################################
def favorite():
    # 인혁이 AI

    with open("jsons/commonGyoyang.json", "r", encoding="utf-8") as f:
        data = json.load(f)
    courses = []
    for i in data:
    ...
    def get_interest():
        ...
        with col1:
            ...
            if st.session_state.get("courses") != None and st.session_state.get(
                ...
                prompt_1 = "나는 "+ str(st.session_state.grades_main)+"학년" +st.session_state.dapartment_main+"학생이야 나는 " + prompt + " 분야에 관심이 있어 " + str(
...
def timeTable():
    ...
    jungong = open("jsons/jungong.json", 'r', encoding='utf-8')  # json 파일 입력
    commonGyoyang = open("jsons/commonGyoyang.json", 'r', encoding='utf-8')  # json 파일 입력
    essentialGyoyang = open("jsons/essentialGyoyang.json", 'r', encoding='utf-8')  # json 파일 입력
    ...
    # print(f"총 가능한 시간표 수: {len(valid_combinations)}")
    # for i in range(len(valid_combinations)):
    #    print_schedule(in_subjects, valid_combinations[i])
    #    print_to_html(in_subjects, valid_combinations[i], file_name="results/result" + str(i) + ".html")
    #    print()

    print_streamlit(in_subjects, valid_combinations)
